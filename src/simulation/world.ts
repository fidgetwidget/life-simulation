...
export class World {
  ...
  constructor(w: number, h: number) {
    ...
    this.motes.fill(0);
    console.debug("World:new", { w, h, v: this.motes });
  }
  ...
  getNeighborValues(i: number, eightWay: boolean = true): number[] {
    const x = Math.floor(i % this.h);
    const y = Math.floor(i / this.h);
  ...
  getNeighborsAt(
    ...
    console.log({ x, y, maxx, maxy, minx, miny, coords });
    return coords.map(({ x, y }) => y * this.h + x);
  }
